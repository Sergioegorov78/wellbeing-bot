...
async function generatePost(item, sourceName, topic) {
  const prompt = `Ты — аналитик, который пишет краткие саммари статей для Telegram-канала об anti-age, биохакинге и longevity.

СТАТЬЯ ДЛЯ АНАЛИЗА:
Заголовок: ${item.title}
Содержание: ${item.description}
Источник: ${sourceName}

ЗАДАЧА:
Прочитай статью и напиши её краткое саммари для Telegram. Только факты из статьи — ничего лишнего.

ФОРМАТ:
1. Первая строка: эмодзи + **жирный заголовок** (точно отражает суть статьи)
2. 2-4 предложения: ключевые факты, цифры, выводы из статьи
3. Если есть — укажи: кто исследовал, сколько участников, какой результат
4. 3-4 хэштега

СТРОГО ЗАПРЕЩЕНО:
- Призывы ("следите за каналом", "присоединяйтесь", "попробуйте")
- Лозунги и общие фразы ("это важно для здоровья", "биохакинг — ключ к долголетию")
- Факты не из статьи
- Упоминание URL
